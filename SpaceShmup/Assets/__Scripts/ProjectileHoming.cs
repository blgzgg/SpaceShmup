...
{
    [Header("Inscribed")]
    public float turnSpeed = 360f;   // Degrees per second the projectile can turn
    ...
    /// <summary>
    /// Called by Weapon when the projectile is spawned.
    /// </summary>
    ...
    void Update()
    {
        // If we had a target, check if we've lost it
        if (hasTarget)
        {
            ...
            {
                BoundsCheck tBnd = target.GetComponent<BoundsCheck>();
                if (tBnd != null && !tBnd.isOnScreen)
                {
                    // Target went off screen – stop homing, keep lastDir
                    hasTarget = false;
                    target = null;
                }
            }
